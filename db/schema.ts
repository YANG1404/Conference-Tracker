...
export const conferenceOrganizations = sqliteTable(
  ...
  (table) => [
    primaryKey({
      columns: [table.conferenceId, table.organizationId, table.roleName],
    }),
  ],
);
...
export const researchFields = sqliteTable(
  'research_fields',
  {
    ...
    parentId: integer('parent_id').references(
      (): AnySQLiteColumn => researchFields.id,
    ),
  ...
  (table) => [
    uniqueIndex('uq_research_fields_acm_ccs_code').on(table.acmCcsCode),
  ],
);
...
export const milestones = sqliteTable(
  'milestones',
  {
    ...
    timeConfirmed: integer('time_confirmed', { mode: 'boolean' })
      .notNull()
      .default(true),
    note: text('note'),
    createdAt: text('created_at')
...
export const conferenceLinks = sqliteTable(
  ...
  (table) => [
    uniqueIndex('uq_conference_links_type_url').on(
      table.conferenceId,
      table.type,
      table.url,
    ),
...
export const collectionRuns = sqliteTable(
  ...
  (table) => [
    index('idx_collection_runs_source').on(table.sourceSiteId, table.startedAt),
  ],
);
...
export const collectionCandidates = sqliteTable(
  'collection_candidates',
  {
    ...
    matchedConferenceId: integer('matched_conference_id').references(
      () => conferences.id,
    ),
...
export const conferenceSources = sqliteTable(
  ...
  (table) => [
    uniqueIndex('uq_conference_sources_url').on(
      table.conferenceId,
      table.sourceUrl,
    ),
  ],
